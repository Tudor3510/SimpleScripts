Function Set-BlueLightReductionSettings {
    ...
    Param (
        ...
        [Parameter(Mandatory=$true)] [ValidateRange(1000, 6500)] [int]$NightColorTemperature
    )
    ...
    Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\CloudStore\Store\DefaultAccount\Current\default$windows.data.bluelightreduction.settings\windows.data.bluelightreduction.settings' -Name 'Data' -Value ([byte[]]$data)
}